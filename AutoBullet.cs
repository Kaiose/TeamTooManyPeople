...
public class Node
{
    ...
    public Node(Rigidbody2D source)
    {
        this.x = source.position.x;
        this.y = source.position.y;
        fcost = 0;
        gcost = 0;
        hcost = 0;
        Parent = null;
...
public class Astar
{
    ...
    public Astar(Rigidbody2D rb, Rigidbody2D target)
    {
    ...
    float Cal_gcost(Node source, int i)
    {
        Vector2 dir;
        RaycastHit2D hit;
        Vector2 pos = new Vector2(source.x, source.y);
        switch (i)
        {
            case 0:
                dir = new Vector2(-1, 1);
                ...
                {
                    source.Child[i].is_possible = false;
                }
                else
                { 
                    source.Child[i].x -= 1;
    ...
    { // 이 부분에서는 유니티가 x,y 를 따라 움직이는 형태로 변해야됨
        while (route.Count != 0)
        {
            ...
            Vector2 pos = new Vector2(way.x, way.y);
            start.rb.MovePosition(pos * Time.deltaTime);
            route.Pop();
            yield return null;
        }
    }
...
public class AutoBullet : MonoBehaviour {
    ...
    bool CheckKey()
    {
        if (Input.GetKeyDown("u")) return true;
        return false;
    }
